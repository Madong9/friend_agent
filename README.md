...
一个可本地运行、可解释、可测试的校园社交 Agent MVP。用户用自然语言描述想找的学习搭子、运动搭子、兴趣伙伴或活动伙伴；Agent 负责理解目标、规划和调用工具，普通程序负责权限、硬过滤、安全规则和最终分数。
...
在校园里，想参加活动、组队比赛、了解某项技术或寻找兴趣搭子，常见路径是先在社交平台上找到对应的群或组织，再在群里提出自己的需求。这条路径看似直接，却容易忽略两类真实困难。

第一是**表达门槛**。不少同学性格内向、慢热，并不是没有参与和社交意愿，而是不习惯在人数较多的活动群、比赛群中公开表达。过去群聊中的匿名表达方式逐渐减少后，他们更缺少一个低压力的需求入口。

第二是**信息发现门槛**。校园兴趣群、赛事群和临时组织高度分散，活动与比赛宣传往往依赖海报、群转发和熟人传播。宣传覆盖不足时，一些本来感兴趣的同学可能直到报名结束也没有发现相关信息；单纯依靠线下宣传，也会受到时间、地点和触达范围限制。

Campus Social Agent 希望提供一个补充方案：用户先在一对一界面中自然地说出“我对什么感兴趣、想参加什么活动或比赛、希望找到怎样的伙伴”，Agent 再帮助补充时间、校区等条件，通过确定性规则给出可解释候选。当前没有合适人选时，需求仍可保留，未来出现符合规则的新候选后再发送站内通知；只有双方都表达兴趣，系统才建立互相匹配。

它并不试图取代社团、群聊和线下宣传，而是降低首次表达成本，让容易沉底、难以开口或暂时没有结果的校园需求获得更持续的连接机会。当前 MVP 已实现找搭子、已录入公开活动查询和持续需求闭环；它不会抓取 QQ、微信等封闭平台，也不宣称覆盖全部校内比赛。未来可在获得学校、社团或赛事组织方授权后，接入标明来源、发布时间和有效期的可信活动信息。
